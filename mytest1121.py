...
def train_data_with_CNN(X_train,Y_train,X_test,Y_test,minibatch_size):
    ...
    #自改X为三层通道的训练数据
    x_input=tf.placeholder(tf.float32,[None,CAPTCHA_IMAGE_HEIGHT,CAPTCHA_IMAGE_WIDHT,1],name='x-input')
    Y = tf.placeholder(tf.float32, [None, CAPTCHA_LEN * CHAR_SET_LEN], name='label-input')
    # dropout,防止过拟合
...
X_train = X_train_total[0:4000, :, :, :]
X_test = X_train_total[4000:5000, :, :, :]
Y_train = Y_train_total[0:4000, :]
Y_test = Y_train_total[4000:5000, :]
